export default {
  ...
  keys: {
    ...
    connectionTestSuccess: '连接正常',
    connectionTestFailed: '连接失败',
    connectionTestRequestFailed: '测试连接失败',
    connectionModelsVisible: '可见模型',
    connectionModelsTested: '已测试模型',
    connectionModelsSkipped: '已跳过',
    connectionTruncated: '仅显示本次抽样测试的前几个模型。',
    connectionModel: '模型',
    connectionLatency: '延迟',
    connectionEndpoint: '端点',
    connectionMessage: '消息',
    connectionNoResults: '没有返回模型探测结果。',
    connectionStatus: {
      success: '正常',
      error: '失败',
      skipped: '跳过'
    ...
    saving: '保存中...',
    noKeysYet: '暂无 API 密钥',
    createFirstKey: '创建第一个 API 密钥，然后发送一次测试请求，即可看到用量和余额变化。',
  ...
  usage: {
    ...
    noRecordsTitle: '还没有使用记录',
    noRecordsDescription: '使用 TokenGate API 密钥发送第一次模型请求。成功请求会在这里显示模型、Token、费用和耗时明细。',
    createOrUseKey: '创建或使用密钥',
    viewQuickstart: '查看快速开始',
  ...
  admin: {
    ...
    users: {
      ...
      testUserBadge: '测试',
      setTestUser: '设为测试用户',
      unsetTestUser: '取消测试用户',
      setTestUserConfirm: "确定要将用户 '{email}' 标记为测试用户吗？该用户的 Stripe 支付将走 sandbox 账户。",
      unsetTestUserConfirm: "确定要取消用户 '{email}' 的测试用户标记吗？该用户的 Stripe 支付将走 live 账户。",
      setTestUserSuccess: '用户已标记为测试用户',
      unsetTestUserSuccess: '用户已取消测试用户标记',
      failedToSetTestUser: '更新测试用户设置失败',
    ...
    accounts: {
      ...
      oauth: {
        ...
        openai: {
          ...
          importantNotice:
            '授权完成后，回调页会分别显示 code 和 state，并提供复制按钮。复制后粘贴回第 3 步即可。',
          step3EnterCode: '输入授权 Code',
          authCodeDesc:
            '授权完成后，从回调页复制 code 并粘贴到下方。完整回调 URL 仍可作为兜底使用。',
          authCode: '授权 Code',
          authCodePlaceholder:
            '粘贴回调页复制的 code\n也支持粘贴完整回调 URL',
          authCodeHint: '从回调页复制 code 和 state；如果粘贴完整回调 URL，系统会自动解析 code/state。',
      ...
      testAllModelsConnection: '测试全部模型连接',
      testingAllModels: '正在测试全部可用模型（{count} 个）...',
      testingModel: '正在测试模型：{model}',
      testAllModelsModelPassed: '{model} 测试通过',
      testAllModelsModelFailed: '{model} 测试失败：{error}',
      testAllModelsSummary: '全部模型测试完成：{passed}/{total} 通过',
      testAllModelsFailedSummary: '全部模型测试完成：{passed}/{total} 通过，{failed} 个失败',
      noModelsAvailableForTest: '没有可测试的模型',
      testModel: '测试模型',
      testPrompt: '提示词："hi"',
    ...
    settings: {
      ...
      payment: {
        ...
        dailyLimit: '每日限额',
	        ...
	        usdCnyRateHint: '管理员配置的人民币支付汇率。TokenGate 套餐、余额和充值金额仍以 USD 计价。',
	        usdCnyRatePreview: '预览：1 USD = {rate} CNY',
  ...
  purchase: {
    title: '充值/订阅',
    description: '选择 Usage Based 充值或 BYO subscription 模式',
    openInNewTab: '新窗口打开',
  ...
  userAccounts: {
    ...
    chooseProviderTitle: '选择账号服务商',
    chooseProviderDescription: '选择本次要授权并绑定到私有分组的服务商。',
    connectProviderTitle: '连接 {provider} 账号',
    ...
    oauthCodePlaceholder: '粘贴回调页复制的 code',
    oauthCodeHint: '回调页会分别显示 code 和 state，并提供复制按钮；完整回调 URL 仍可作为兜底自动识别。',
    ...
    deleteFailed: '移除账号失败',
    emptyTitle: '暂无已连接账号',
    emptyDescription: '连接您自己的 AI 账号后，会生成一个只有您的 API Key 可使用的私有分组。',
    ...
    providers: {
      ...
      geminiDescription: '使用 Gemini 账号作为私有上游。'
    },
    columns: {
      name: '名称',
      platform: '平台',
  ...
  payment: {
    ...
    summary: {
      ...
      byoValue: '自有账号',
      byoHint: '订阅后，TokenGate 会通过您绑定的上游账号转发请求。',
      byoStepOneTitle: '绑定账号',
      byoStepOneDesc: '连接您自己的受支持 AI 服务账号。',
      byoStepTwoTitle: '订阅访问',
      byoStepTwoDesc: '保持 BYO 路由对 API Key 可用。',
      byoStepThreeTitle: '消耗自有额度',
      byoStepThreeDesc: '请求消耗您上游账号里的 token 或额度。',
    },
    modeTabs: {
      usageBased: {
        label: 'Usage Based 模式',
        description: '充值后按 token 用量扣费',
      },
      byoSubscription: {
        label: 'BYO subscription 模式',
        description: '订阅后使用自己的账号',
      },
    },
    modeDescriptions: {
      usageBased: {
        title: 'Usage Based 模式使用 TokenGate 余额',
        desc: '用户先充值余额，随后每次请求都会按照模型价格和实际 token、图片或视频用量从余额中扣费。',
      },
      byoSubscription: {
        title: 'BYO subscription 模式使用您自己的 AI 账号',
        desc: '用户绑定自己的上游账号并完成订阅后，请求会消耗该账号里的 token 或额度，而不是消耗 TokenGate 余额。',
      },
    },
	    amountLabel: '充值金额',
	    usdAmount: 'USD 金额',
    ...
    fee: '手续费',
	    actualPay: '实付金额',
	    usdCnyRatePreview: '人民币支付使用管理员配置汇率：1 USD = {rate} CNY',
	    createOrder: '确认支付',
    ...
    balanceExplainer: {
      title: '余额就是您的账务钱包',
      desc: '余额用于按量请求扣费。文本模型按 token 用量计费，图片和视频模型按各自原生单位计费。',
    },
    currentBalance: '可用余额',
    groupFallback: '分组 #{id}',
    ...
    planExplainer: {
      title: '订阅后通过 TokenGate 使用您绑定的账号',
      stepOneTitle: '绑定账号',
      stepOneDesc: '使用 BYO 分组前，先连接一个受支持的 AI 服务账号。',
      stepTwoTitle: '保持访问',
      stepTwoDesc: '订阅会让这条 BYO 路由持续对您的 API Key 可用。',
      stepThreeTitle: '消耗自有额度',
      stepThreeDesc: '请求由您的上游账号承载，不会按用量扣除 TokenGate 余额。',
    },
    ...
    admin: {
      ...
      billingProviderInternal: '内部开通',
      billingProviderStripe: 'Stripe',
      billingModeFixedPeriod: '固定周期',
      billingModeSubscription: '订阅',
      stripeLivePriceId: 'Live Price ID',
      stripeSandboxPriceId: 'Sandbox Price ID',
      stripeTrialDays: '试用天数',
      forSale: '上架状态',
      onSale: '上架',
      offSale: '下架',
      group: '分组',

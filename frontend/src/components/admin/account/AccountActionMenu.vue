      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="supportsModelRefresh" @click="$emit('refresh-models', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-cyan-600 hover:bg-gray-100 dark:hover:bg-dark-700">
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'reauth', 'refresh-token', 'refresh-models', 'recover-state', 'reset-quota', 'set-privacy'])
const { t } = useI18n()

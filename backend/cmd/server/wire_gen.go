...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	groupRepository := repository.NewGroupRepositoryConcrete(client, db)
	...
	accountRepository := repository.NewAccountRepositoryConcrete(client, db, schedulerCache, secretEncryptor)
	...
	byoSubscriptionEntitlementChecker := repository.ProvideBYOSubscriptionEntitlementChecker(userSubscriptionRepository)
	connectedAccountService := service.NewConnectedAccountService(accountRepository, groupRepository, openAIOAuthService, oAuthService, geminiOAuthService, byoSubscriptionEntitlementChecker)
	...
	connectedAccountHandler := handler.NewConnectedAccountHandler(connectedAccountService, accountTestService, rateLimitService)
	...
	globalModelPricingOverrideRepository := repository.NewGlobalModelPricingRepository(db)
	modelPricingResolver := service.NewModelPricingResolverWithGlobalOverrides(channelService, billingService, globalModelPricingOverrideRepository)
	...
	globalModelPricingService := service.NewGlobalModelPricingService(globalModelPricingOverrideRepository, billingService)
	...
	stripeBillingService := service.NewStripeBillingService(paymentService, paymentConfigService, userRepository, userSubscriptionRepository, byoSubscriptionEntitlementChecker, accountRepository, billingCacheService, apiKeyAuthCacheInvalidator)
	handlerPaymentHandler := handler.ProvidePaymentHandler(paymentService, paymentConfigService, channelService, stripeBillingService)
	paymentWebhookHandler := handler.ProvidePaymentWebhookHandler(paymentService, registry, stripeBillingService)
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, connectedAccountHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	subscriptionExpiryService := service.ProvideSubscriptionExpiryService(userSubscriptionRepository, accountRepository)

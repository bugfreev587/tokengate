...
func (s *APIKeyService) canUserBindGroup(ctx context.Context, user *User, group *Group) bool {
	if allowed, ok := canUserBindConnectedAccountGroup(user, group); ok {
		return allowed
	}
	// 订阅类型分组：需要有效订阅
...
func canUserBindConnectedAccountGroup(user *User, group *Group) (bool, bool) {
	if group == nil || !group.IsUserOwnedConnectedAccount() {
		return false, false
	}
	if user == nil || user.ID <= 0 || group.OwnerUserID == nil {
		return false, true
	}
	return *group.OwnerUserID == user.ID, true
}

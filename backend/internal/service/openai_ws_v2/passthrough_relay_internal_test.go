...
func TestParseUsageAndEnrichCoverage(t *testing.T) {
	...
	parseUsageAndAccumulate(state, []byte(`{"type":"response.completed","response":{"usage":{"input_tokens":2,"output_tokens":1,"input_tokens_details":{"cached_tokens":1,"cache_write_tokens":2}}}}`), "response.completed", nil)
	...
	require.Equal(t, 2, state.usage.CacheCreationInputTokens)

...
TokenGate 内置支付系统，支持用户自助充值，无需部署独立的支付服务。
...
> **易支付服务商说明**：兼容易支付协议的服务商会因国家/地区、结算币种、合规要求和风控策略而差异很大。TokenGate 不默认背书任何第三方支付服务商。生产启用前，请自行验证商户资质要求、退款规则、Webhook 稳定性和结算条款。
>
> TokenGate 当前 V1 支付目标是 Stripe + 支付宝 + 微信支付。Stripe 仍然适合作为国际 beta 的第一个验证渠道，因为 test mode、Webhook、退款和合规流程更可预测。支付宝和微信支付需要在商户凭证、回调地址、签名验签都端到端验证通过后再开放。
...
| 对比项 | Sub2ApiPay | 内置支付 |
|--------|-----------|---------|
| 部署方式 | 独立服务（Next.js + PostgreSQL） | 内置于 TokenGate，无需额外部署 |
| 支付方式 | EasyPay、支付宝、微信、Stripe | 相同 |
| 配置方式 | 环境变量 + 独立管理后台 | TokenGate 管理后台内统一配置 |
| 充值对接 | 通过 Admin API 回调 | 内部直接处理，更可靠 |
| 订阅套餐 | 支持 | 暂不支持（计划中） |
| 订单管理 | 独立管理界面 | 集成在 TokenGate 管理后台 |

### 迁移步骤

1. 在 TokenGate 管理后台启用支付并配置服务商（使用相同的支付凭证）
2. 更新 Webhook 回调地址为 TokenGate 的回调地址
3. 确认新订单通过内置支付正常处理
4. 停用 Sub2ApiPay 服务
